...
fn set_scaling_recursive(key: &RegKey, count: &mut u32) {
    if key.get_value::<u32, _>("Scaling").is_ok() && key.set_value("Scaling", &3u32).is_ok() {
        *count += 1;
    }
...
pub fn set_scaling_fullscreen() -> (bool, String) {
    ...
    let root = match hklm.open_subkey_with_flags(path, KEY_READ | KEY_SET_VALUE) {
        ...
        Err(_) => {
            return (
                false,
                "NVIDIA scaling: config key not found (needs admin?)".into(),
            )
        }
    };
    let mut count = 0u32;
    ...
    if count > 0 {
        (
            true,
            format!("NVIDIA scaling set to Full-screen ({} display(s))", count),
        )
    } else {
